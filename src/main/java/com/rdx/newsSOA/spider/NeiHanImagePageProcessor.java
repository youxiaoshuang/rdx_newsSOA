...
public class NeiHanImagePageProcessor implements PageProcessor {
    @Autowired
...
//        }
    ...
    public List<TouTiaoNewModel> getNewsList(Page page) {
        ...
        for (int i = 0; i < dataArray.size(); i++) {
            ...
            String image_url = (String) group.getJSONObject( "large_image" ).getJSONArray( "url_list" ).getJSONObject( 0 ).get( "url" );
            ...
            yFile.setUrl( image_url );
            String fileType = image_url.substring( image_url.lastIndexOf( "." ), image_url.length() );
    ...
    public void run(NeiHanImagePageProcessor neiHanHotPageProcessor, String url) {
        Spider.create( neiHanHotPageProcessor ).addUrl( url ).thread( 1 ).run();
    }

    public static void main(String[] args) {
        Long dateTime = new Date().getTime();
        Spider.create( new NeiHanImagePageProcessor() ).addUrl("http://m.neihanshequ.com/pic/?is_json=1&app_name=neihanshequ_web&min_time=1480926805&csrfmiddlewaretoken=783a7367ddad6fa93161b76e649a59c0").thread( 1 ).run();
    }
}